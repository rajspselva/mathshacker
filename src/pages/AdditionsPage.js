...
export default function AdditionsPage() {
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - additions.length) : 0;
    ...
    <>
      ...
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          ...
          <Typography variant="h4" gutterBottom>
            Remaining: {100 - additions.length}
          </Typography>
          <Typography variant="h4" gutterBottom>
            Correct Anwser: {additions.filter((f) => f.result === true).length}
          </Typography>
          <Typography variant="h4" gutterBottom>
            Accuracy: {(additions.length !== 0 ? additions.filter((f) => f.result === true).length / additions.length * 100 : 0).toFixed(2)}%
          </Typography>
        </Stack>
        <Card>
          ...
          <Scrollbar>
            <TableContainer sx={{ minWidth: 800 }}>
              <Table>
                ...
                <TableBody>
                  {additions.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => {
